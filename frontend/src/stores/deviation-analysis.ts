...
export const useAnalysisStore = defineStore('deviation-analyses', () => {
  ...
  async function reviewPhase(phase: string, disposition: PhaseDisposition, note: string) {
    if (!selected.value) return
    selected.value = await submitPhaseReview(selected.value.id, phase, disposition, note)
    await load()
  }
